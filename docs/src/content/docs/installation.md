...
To install linkding using Docker, clone the repository and build the image locally (no image is pulled from a registry):

```shell
git clone https://github.com/runawaydevil/link-ding.git
cd link-ding
docker build -f docker/default.Dockerfile -t linkding:latest .
docker run --name linkding -p 9090:9090 -v {host-data-folder}:/etc/linkding/data -d linkding:latest
```
...
To upgrade the installation to a new version, pull the latest code, rebuild the image, remove the existing container, and start a new container using the same command that you used above. There is a [shell script](https://github.com/runawaydevil/link-ding/blob/main/install-linkding.sh) available to automate these steps. The script can be configured using environment variables, or you can just modify it.

To complete the setup, you still have to [create an initial user](#user-setup), so that you can access your installation.

##  Using Docker Compose

To install linkding using [Docker Compose](https://docs.docker.com/compose/), clone the repository (or download the [`docker-compose.yml`](https://github.com/runawaydevil/link-ding/blob/main/docker-compose.yml) and [`.env.sample`](https://github.com/runawaydevil/link-ding/blob/main/.env.sample) files), rename `.env.sample` to `.env`, configure the parameters, and then run:
```shell
docker-compose up -d
```
The compose file builds the image locally; no image is pulled from a registry.
